...
Gorilla OpenFunctions v2（7B模型）相比前代，增加了几个关键能力：
多函数调用 (Multiple functions): 能从多个函数中选择需要的一个或多个来调用。
并行函数调用 (Parallel functions): 能在一次请求中，用不同的参数多次调用同一个函数。
例子： "查询波士顿和旧金山的天气" -> 会并行调用 get_current_weather 两次。
混合调用: 同时支持上述两种情况。
相关性检测 (Relevance detection): 能判断用户是在普通聊天，还是在请求函数调用。
多语言数据类型支持:
Python: 支持 string, number, boolean, list, tuple, dict 等。
JAVA: 支持 int, float, boolean, Array, ArrayList, HashMap 等。
JavaScript: 支持 String, Number, Boolean, Array, Date 等。
REST: 原生支持 REST API。
