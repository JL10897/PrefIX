...
def insert_callback(service, key):
    """供 main() 的 -insert_creds 调用：打印提示并把 service-key 写入全局凭据存储。"""
    print(SUCCESS)
...
def list_callback():
    """被 main() 的 -list_creds 触发，用来查看 insert/remove 操作共享的凭据状态。"""
    print(list_creds(target = CREDS_FOLDER_PATH))

def restful_callback(prompt, generate_mode):
    """main() 在 execute -type rest 时调用：依赖 get_config/default_config 决定重试模型与撤销策略。"""
...
def initialize_user_config():
    """供 main() 启动时调用，确保 update/get_config 有落地文件可读写。"""
...
def update_user_config(key, value):
    """由 main() 的 -set_config 使用，写入后 get_config/fs/db/rest 回调都能读取最新值。"""
...
def get_config(key):
    """被所有执行型回调（rest/db/fs）读取，用来共享 initialize/update_user_config 维护的设置。"""
    with open(USER_CONFIG_PATH, 'r') as j:
        config = json.load(j)
    return config[key]

def authorize_callback(services):
    """main() 解析 -authorize 后调用，补充 insert/list/remove 凭据相关回调的另一入口。"""
...
def fs_callback(prompt, generate_mode):
    """main() 在 execute -type fs 时调用，与 db/rest 共享 ExecutionEngine 逻辑并依赖 get_config 配置。"""
...
def remove_creds_callback(services):
    """main() 的 -remove_creds 控制入口，用来配合 insert/list 掌控凭据生命周期。"""
    try:
...
def db_callback(prompt, generate_mode):
    """main() 在 execute -type db 时调用，与 fs/rest 共用 get_config/default_config 的选项。"""
    config = {
...
def exit_with_help_message(parser):
    """供 ArgumentParser/_HelpAction 共用的帮助输出，解释 main() 注册的所有子命令。"""
...
class _HelpAction(argparse._HelpAction):
    """自定义 help action，被 main() 注册为 --help，调用 exit_with_help_message。"""

    def __call__(self, parser, namespace, values, option_string=None):
        """直接复用 exit_with_help_message，让 CLI 所有入口保持同样帮助信息。"""
        exit_with_help_message(parser)

class ArgumentParser(argparse.ArgumentParser):
    """main() 使用的解析器封装，统一把错误引导至 exit_with_help_message。"""

    def error(self, message):
        """覆盖默认 error，从而与 _HelpAction 一样输出完整帮助内容。"""
        exit_with_help_message(self)


def main():
    """CLI 入口：调用 initialize_user_config、解析参数并路由到 rest/db/fs/凭据相关回调。"""
